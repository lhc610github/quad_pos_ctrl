...
int main(int argc,char** argv){
    ...
    // straight line
        start.makeStartOrEnd(Eigen::Vector3d(0,0,-1.5),derivative_to_optimize);
        vertices.push_back(start);

        middle.addConstraint(mav_trajectory_generation::derivative_order::POSITION,Eigen::Vector3d(1,1,-1.5));
        vertices.push_back(middle);

        end.makeStartOrEnd(Eigen::Vector3d(0,0,-1.5),derivative_to_optimize);
        vertices.push_back(end);
    // curve 3 in paper
        /*start.makeStartOrEnd(Eigen::Vector3d(-1.2,0,2),derivative_to_optimize);
        vertices.push_back(start);
    ...
    const double magic_fabian_constant = -0.5; // A tuning parameter
